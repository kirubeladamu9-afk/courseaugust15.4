...
const AssessmentBuilder: FC = () => {
  ...
  useEffect(() => {
    api.get<{ grades: string[] }>('/api/teacher/assigned-grades', { withCredentials: true })
      .then(({ data }) => { setAssignedGrades(data.grades); setClassName(data.grades[0] || '') })
      .catch(() => setError('Unable to load your assigned grades. Please try again.'))
  ...
  const addQuestion = () => setQuestions((current) => [...current, newQuestion(questionType)])
  ...
  const saveAssessment = async () => {
    ...
    setError('')
    try {
      await api.post('/api/teacher/assessments', { title, className, questions }, { withCredentials: true })
  ...
  return <Stack spacing={2}>
    <Paper elevation={0} sx={{ p: { xs: 2, md: 3 }, borderRadius: 3 }}><Grid container spacing={2}><Grid item xs={12} md={7}><TextField fullWidth label="Title" value={title} onChange={(event) => setTitle(event.target.value)} /></Grid><Grid item xs={12} md={5}><Select fullWidth value={className} disabled={gradesLoading || !assignedGrades.length} onChange={(event) => setClassName(event.target.value)} displayEmpty aria-label="Assigned grade"><MenuItem value="" disabled>{gradesLoading ? 'Loading assigned grades...' : assignedGrades.length ? 'Select an assigned grade' : 'No assigned grades'}</MenuItem>{assignedGrades.map((grade) => <MenuItem key={grade} value={grade}>{grade}</MenuItem>)}</Select></Grid></Grid></Paper>
    <Paper elevation={0} sx={{ p: { xs: 2, md: 3 }, borderRadius: 3 }}><Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} alignItems={{ xs: 'stretch', sm: 'center' }} justifyContent="space-between"><Box><Typography variant="h5">Questions</Typography><Typography variant="body2" color="text.secondary">Choose a type before adding each question.</Typography></Box><Stack direction="row" spacing={1}><Select size="small" value={questionType} onChange={(event) => setQuestionType(event.target.value as QuestionType)} aria-label="Question Type">{Object.entries(questionTypeLabels).map(([value, label]) => <MenuItem key={value} value={value}>{label}</MenuItem>)}</Select><Button variant="contained" startIcon={<AddRounded />} onClick={addQuestion} disabled={!assignedGrades.length}>Add Question</Button></Stack></Stack>
