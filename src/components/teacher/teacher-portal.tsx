...
const AssessmentBuilder: FC = () => {
  ...
  const saveAssessment = async () => {
    if (!title.trim() || !questions.length || questions.some((question) => !question.prompt.trim() || !question.points || (question.type !== 'fill-blank' && (Array.isArray(question.correctAnswer) ? !question.correctAnswer.length : question.correctAnswer === '')))) {
  ...
  return <Stack spacing={2}>
    <Paper elevation={0} sx={{ p: { xs: 2, md: 3 }, borderRadius: 3 }}><Grid container spacing={2}><Grid item xs={12} md={7}><TextField fullWidth label="Title" value={title} onChange={(event) => setTitle(event.target.value)} /></Grid><Grid item xs={12} md={5}><Select fullWidth value={className} onChange={(event) => setClassName(event.target.value)} aria-label="Class"><MenuItem value="Grade 7">Grade 7</MenuItem><MenuItem value="Grade 8">Grade 8</MenuItem><MenuItem value="Grade 9">Grade 9</MenuItem></Select></Grid></Grid></Paper>
    <Paper elevation={0} sx={{ p: { xs: 2, md: 3 }, borderRadius: 3 }}><Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} alignItems={{ xs: 'stretch', sm: 'center' }} justifyContent="space-between"><Box><Typography variant="h5">Questions</Typography><Typography variant="body2" color="text.secondary">Choose a type before adding each question.</Typography></Box><Stack direction="row" spacing={1}><Select size="small" value={questionType} onChange={(event) => setQuestionType(event.target.value as QuestionType)} aria-label="Question Type">{Object.entries(questionTypeLabels).map(([value, label]) => <MenuItem key={value} value={value}>{label}</MenuItem>)}</Select><Button variant="contained" startIcon={<AddRounded />} onClick={addQuestion}>Add Question</Button></Stack></Stack>
