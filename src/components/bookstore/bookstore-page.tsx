...
export const BookstorePage: FC = () => {
  ...
  return <Box sx={{ minHeight: '100vh', backgroundColor: 'background.default', py: { xs: 4, md: 7 } }}><Box sx={{ maxWidth: 1180, mx: 'auto', px: 3 }}><Button onClick={() => navigateTo('/')} sx={{ mb: 2 }}>← Back to Coursespace</Button><Box sx={{ mb: 4 }}><Typography variant="overline" color="primary" sx={{ fontWeight: 800, letterSpacing: 1.1 }}>Digital resources</Typography><Typography variant="h3" sx={{ fontWeight: 800, mb: 1 }}>CourseSpace Bookstore</Typography><Typography color="text.secondary">Books, workbooks, and resources to support every stage of learning.</Typography></Box>{isLoading ? <Box sx={{ display: 'flex', justifyContent: 'center', py: 10 }}><CircularProgress /></Box> : items.length === 0 ? <Paper variant="outlined" sx={{ p: 6, textAlign: 'center' }}><Typography variant="h6">New resources are coming soon.</Typography></Paper> : <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)', md: 'repeat(3, 1fr)' }, gap: 3 }}>{items.map((item) => <Card key={item.id} variant="outlined" sx={{ display: 'flex', flexDirection: 'column' }}><Box component="img" src={item.coverData} alt="" sx={{ width: '100%', height: 220, objectFit: 'cover', backgroundColor: 'action.hover' }} /><CardContent sx={{ display: 'flex', flexDirection: 'column', gap: 1.25, flex: 1 }}><Chip label={item.category} size="small" sx={{ alignSelf: 'flex-start' }} /><Typography variant="h6" fontWeight={700}>{item.title}</Typography><Typography variant="body2" color="text.secondary" sx={{ flex: 1 }}>{item.description}</Typography><Divider /><Typography fontWeight={800}>{formatAmount(item.price, item.currency)}</Typography>{owned.has(item.id) ? <Button variant="contained" startIcon={<DownloadOutlinedIcon />} onClick={() => { window.location.assign(`/api/bookstore-items/${item.id}/download`) }}>Download {item.fileName}</Button> : <Button variant="contained" disabled={purchasingId === item.id} onClick={() => void buy(item)}>{purchasingId === item.id ? 'Opening checkout...' : 'Buy now'}</Button>}</CardContent></Card>)}</Box>}</Box></Box>
}
