...
export const BookstorePage: FC<BookstorePageProps> = ({ darkMode, onToggleDarkMode }) => {
  ...
  const buy = async (item: BookstoreItem) => {
    ...
    if (!user) {
      const enteredEmail = window.prompt('Enter your email to continue with checkout:')
      if (enteredEmail === null) return
      guestEmail = enteredEmail.trim()
      if (guestEmail.length > 254 || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(guestEmail)) {
        toast.add({ title: 'Valid email required', description: 'Enter a valid email address to continue checkout.', type: 'error' })
        return
      }
    }
    setPurchasingId(item.id)
    try { const checkout = await createBookstoreChapaCheckout(item.id, guestEmail ? { email: guestEmail } : undefined); if (checkout.mode === 'test') { await completeTestPayment(checkout.paymentReference, 'paid'); reloadPurchases(); toast.add({ title: 'Purchase complete', description: `${item.title} is now in your library.`, type: 'success' }) } else window.location.assign(checkout.checkoutUrl) } catch (error) { toast.add({ title: 'Unable to start checkout', description: error instanceof Error ? error.message : 'Please try again.', type: 'error' }) } finally { setPurchasingId(null) }
  }
  const owned = new Set(purchases.map((purchase) => purchase.itemId))
  return <Box sx={{ minHeight: '100vh', backgroundColor: 'background.default' }}><Header darkMode={darkMode} onSignIn={() => navigateTo('/')} onToggleDarkMode={onToggleDarkMode} /><Box component="main" sx={{ py: { xs: 4, md: 7 } }}><Box sx={{ maxWidth: 1180, mx: 'auto', px: 3 }}><Box sx={{ mb: 4 }}><Typography variant="overline" color="primary" sx={{ fontWeight: 800, letterSpacing: 1.1 }}>Digital resources</Typography><Typography variant="h3" sx={{ fontWeight: 800, mb: 1 }}>CourseSpace Bookstore</Typography><Typography color="text.secondary">Books, workbooks, and resources to support every stage of learning.</Typography></Box>{isLoading ? <Box sx={{ display: 'flex', justifyContent: 'center', py: 10 }}><CircularProgress /></Box> : items.length === 0 ? <Paper variant="outlined" sx={{ p: 6, textAlign: 'center' }}><Typography variant="h6">New resources are coming soon.</Typography></Paper> : <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)', md: 'repeat(3, 1fr)' }, gridAutoRows: '1fr', alignItems: 'stretch', gap: 3 }}>{items.map((item) => { const isOwned = owned.has(item.id); const isPurchasing = purchasingId === item.id; const actionLabel = isOwned ? `Download ${item.fileName}` : isPurchasing ? 'Opening checkout...' : `Purchase ${item.title}`; const takeAction = () => { if (isOwned) window.location.assign(`/api/bookstore-items/${item.id}/download`); else void buy(item) }; return <Card key={item.id} sx={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', borderRadius: 3, boxShadow: '0 8px 24px rgba(25, 70, 47, 0.10)', transition: 'transform 180ms ease, box-shadow 180ms ease', '&:hover, &:focus-within': { transform: 'translateY(-6px)', boxShadow: '0 18px 36px rgba(25, 70, 47, 0.18)', '& .bookstore-card-overlay': { opacity: 1 }, '& .bookstore-card-details': { maxHeight: 190, opacity: 1, mt: 1.5 } } }}><Box component="button" type="button" disabled={isPurchasing} onClick={takeAction} aria-label={actionLabel} sx={{ position: 'relative', display: 'block', width: '100%', p: 0, border: 0, background: 'transparent', cursor: isPurchasing ? 'wait' : 'pointer', overflow: 'hidden', '&:focus-visible': { outline: '3px solid', outlineColor: 'primary.main', outlineOffset: -3 } }}><Box component="img" src={item.coverData} alt={`Cover of ${item.title}`} sx={{ display: 'block', width: '100%', height: 250, objectFit: 'cover', backgroundColor: 'action.hover', transition: 'transform 240ms ease', '.MuiCard-root:hover &, button:focus-visible &': { transform: 'scale(1.04)' } }} /><Box className="bookstore-card-overlay" sx={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 1, color: 'common.white', backgroundColor: 'rgba(27, 112, 69, 0.84)', opacity: 0, transition: 'opacity 180ms ease' }}><Box sx={{ display: 'grid', placeItems: 'center', width: 54, height: 54, borderRadius: '50%', backgroundColor: 'common.white', color: 'primary.main', boxShadow: 3 }}><DownloadOutlinedIcon /></Box><Typography variant="button" sx={{ color: 'inherit', fontWeight: 800 }}>{isPurchasing ? 'Opening checkout...' : isOwned ? 'Download resource' : 'View & purchase'}</Typography></Box></Box><CardContent sx={{ display: 'flex', flexDirection: 'column', flex: 1, p: 2.25, '&:last-child': { pb: 2.25 } }}><Box sx={{ display: 'flex', alignItems: 'baseline', gap: 1, justifyContent: 'space-between' }}><Typography variant="h6" fontWeight={800} noWrap sx={{ minWidth: 0, flex: 1 }}>{item.title}</Typography><Typography variant="caption" color="text.secondary" noWrap>{item.category}</Typography><Typography fontWeight={900} color="primary.main" noWrap>{formatAmount(item.price, item.currency)}</Typography></Box><Divider sx={{ my: 1.5 }} /><Box className="bookstore-card-details" sx={{ display: 'grid', gridTemplateRows: '1fr', gap: 1.25, maxHeight: 0, mt: 0, opacity: 0, overflow: 'hidden', transition: 'max-height 180ms ease, opacity 180ms ease, margin 180ms ease' }}><Typography variant="body2" color="text.secondary">{item.description}</Typography><Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1 }}><Typography variant="caption" color="text.secondary" noWrap title={item.fileName}>{item.fileName} · {formatSize(item.fileSizeBytes)}</Typography><Stack component="ul" direction="row" spacing={0.5} aria-label="Resource palette" sx={{ m: 0, p: 0, listStyle: 'none' }}>{[['#24744a', 'Forest green'], ['#f5bd47', 'Golden yellow'], ['#db7158', 'Terracotta'], ['#4974a5', 'Slate blue']].map(([color, label]) => <Box component="li" key={color} aria-label={`Palette color: ${label}`} sx={{ width: 12, height: 12, borderRadius: '50%', backgroundColor: color, border: '1px solid', borderColor: 'background.paper' }} />)}</Stack></Box></Box></CardContent></Card>})}</Box>}</Box></Box><Footer /></Box>
}
