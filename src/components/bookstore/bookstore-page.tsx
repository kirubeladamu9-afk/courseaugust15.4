...
export const BookstorePage: FC<BookstorePageProps> = ({ darkMode, onToggleDarkMode }) => {
  ...
  const buy = async (item: BookstoreItem) => {
    if (!user) return toast.add({ title: 'Sign in to purchase', description: 'Sign in from the home page, then return to the bookstore.', type: 'info' })
    setPurchasingId(item.id)
    try { const checkout = await createBookstoreChapaCheckout(item.id); if (checkout.mode === 'test') { await completeTestPayment(checkout.paymentReference, 'paid'); reloadPurchases(); toast.add({ title: 'Purchase complete', description: `${item.title} is now in your library.`, type: 'success' }) } else window.location.assign(checkout.checkoutUrl) } catch (error) { toast.add({ title: 'Unable to start checkout', description: error instanceof Error ? error.message : 'Please try again.', type: 'error' }) } finally { setPurchasingId(null) }
  }
