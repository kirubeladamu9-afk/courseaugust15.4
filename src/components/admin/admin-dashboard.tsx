...
const getLessonLabel = (lesson: AdminLesson) => lesson.type === 'video' ? (lesson.duration ? formatDuration(lesson.duration) : 'Processing') : lesson.type === 'article' ? 'Article' : lesson.type === 'quiz' ? 'Quiz' : lesson.type === 'practice' ? 'Practice' : 'Live'
...
export const CourseEditor: FC<{ course: AdminCourse; onChange: (course: AdminCourse) => void; isClassLinked?: boolean; curriculumOnly?: boolean; allowedLessonTypes?: LessonType[] }> = ({ course, onChange, isClassLinked = false, curriculumOnly = false, allowedLessonTypes }) => {
  ...
  const openLessonPanel = (moduleId: number, lesson: AdminLesson, isNew: boolean) => {
    setAddingLessonModuleId(null)
    setVideoUploadProgress(0)
    setLessonPanel({ moduleId, lesson: { ...lesson, resources: [...lesson.resources], quizQuestions: lesson.quizQuestions?.map((question) => ({ ...question, options: [...question.options] })), practiceQuestions: lesson.practiceQuestions?.map((question) => ({ ...question, options: [...question.options] })) }, isNew })
  }
