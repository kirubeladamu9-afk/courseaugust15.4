...
export const CourseEditor: FC<{ course: AdminCourse; onChange: (course: AdminCourse) => void; isClassLinked?: boolean; curriculumOnly?: boolean; liveLessonSettings?: LiveLessonSettings }> = ({ course, onChange, isClassLinked = false, curriculumOnly = false, liveLessonSettings }) => {
  ...
  return (
    <Paper id="course-curriculum-editor" elevation={0} sx={{ mt: 3, p: 2.5, border: 1, borderColor: 'divider', scrollMarginTop: 24 }}>
      ...
      {lessonPanel && <LessonDetailsPanel lessonPanel={lessonPanel} liveLessonSettings={liveLessonSettings} updateLessonDraft={updateLessonDraft} onClose={() => setLessonPanel(null)} onSave={saveLesson} onVideoFile={handleVideoFile} onAddResources={addResources} videoUploadProgress={videoUploadProgress} disabled={cannotSaveLesson} />}
    </Paper>
  )
}

const LessonDetailsPanel: FC<{ lessonPanel: LessonPanelState; liveLessonSettings?: LiveLessonSettings; updateLessonDraft: (lesson: AdminLesson) => void; onClose: () => void; onSave: () => void; onVideoFile: (file: File) => void; onAddResources: (files: FileList | null) => void; videoUploadProgress: number; disabled: boolean }> = ({ lessonPanel, liveLessonSettings, updateLessonDraft, onClose, onSave, onVideoFile, onAddResources, videoUploadProgress, disabled }) => {
