...
const ViolationsPage: FC = () => {
  ...
  return <>
    ...
    {isLoading ? <Box sx={{ display: 'flex', justifyContent: 'center', py: 8 }}><CircularProgress aria-label="Loading quiz violations" /></Box> : <AdminDataTable rows={violations} columns={[{ key: 'studentName', label: 'Student' }, { key: 'courseTitle', label: 'Course' }, { key: 'lessonTitle', label: 'Quiz' }, { key: 'violations', label: 'Violations', render: (value) => (value as AdminQuizViolation['violations']).map(formatViolation).join(', ') }, { key: 'disqualified', label: 'Status', render: (_, row) => <Chip label={row.retakeApproved ? 'Retake approved' : row.disqualified ? 'Disqualified' : 'Flagged'} color={row.retakeApproved ? 'success' : 'warning'} size="small" /> }]} searchPlaceholder="Search violations" searchKeys={['studentName', 'studentEmail', 'courseTitle']} actions={(row) => row.disqualified && !row.retakeApproved ? <MuiButton size="small" variant="contained" onClick={() => void approveRetake(row.id)}>Approve retake</MuiButton> : null} />}
  </>
}
