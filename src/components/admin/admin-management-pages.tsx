...
const additionalSections: Record<string, SectionConfig> = {
  ...
  'learning-materials': makeSection('Learning Materials', 'Review documents uploaded by teachers and shared with classes or selected students.', '', ['Material', 'File', 'File type', 'Subject', 'Class', 'Audience', 'Description', 'Uploaded date', 'Downloadable', 'Status'], 'No materials yet'),
  ...
  useEffect(() => {
    ...
    api.get<{ records: AdminRecord[] }>(`/api/admin/${section}`, { withCredentials: true })
      .then(({ data }) => setRecords(section === 'learning-materials' ? data.records.map((record) => {
        ...
        return { ...record, data: { Material: record.title, File: String(materialData.fileName || '—'), 'File type': String(materialData.fileExtension || '—').toUpperCase(), Subject: String(materialData.subjectName || '—'), Class: String(materialData.className || '—'), Audience: materialData.assignmentScope === 'Whole Class' ? 'Whole class' : `${studentCount} selected student${studentCount === 1 ? '' : 's'}`, Description: String(materialData.description || '—'), 'Uploaded date': record.createdAt ? new Intl.DateTimeFormat(undefined, { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(record.createdAt)) : '—', Downloadable: 'Yes', materialId: record.id } }
      }) : data.records))
