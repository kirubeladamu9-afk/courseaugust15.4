...
const sections: Record<string, SectionConfig> = {
  ...
  subjects: { title: 'Subjects', description: 'Organize the subjects available in your academic program.', action: 'Add Subject', columns: ['Subject', 'Grade levels', 'Teachers', 'Status', 'Actions'], rows: [['Mathematics', '6–9', '32', 'Published'], ['Science', '6–9', '28', 'Published'], ['English Language', '6–9', '35', 'Published'], ['World History', '7–9', '18', 'Draft']], metrics: [['Total subjects', '18', '+2.0%'], ['Published', '15', '+7.1%'], ['Drafts', '3', '-14.3%']], createFields: [{ name: 'subject', label: 'Subject', required: true }, { name: 'gradeLevels', label: 'Grade levels', required: true }, { name: 'teachers', label: 'Teachers', required: true }, { name: 'status', label: 'Status', options: ['Draft', 'Published'], required: true }] },
...
const additionalSections: Record<string, SectionConfig> = {
  ...
  'learning-materials': makeSection('Learning Materials', 'Review documents uploaded by teachers and shared with classes or selected students.', '', ['Material', 'File', 'File type', 'Subject', 'Class', 'Uploaded by', 'Audience', 'Description', 'Uploaded date', 'Downloadable', 'Status'], 'No materials yet'),
...
const adminMaterialFileUrl = (materialId: string) => `${api.defaults.baseURL || ''}/api/admin/learning-materials/${materialId}/file`
...
type AdminRecord = { id: string; title: string; data: Record<string, string>; status: string; createdAt?: string; questions?: AdminQuestion[]; sourceType?: 'teacher' | 'student' | 'guardian' | 'user'; sourceId?: string; userId?: string; studentId?: string }
type ReportStudent = { id: string; fullName: string; gradeLevel: string; classSection: string; academicYear: string; status: string; createdAt?: string; guardianLinks: { guardian: { name: string } }[] }
type ReportUser = { title: string; data: Record<string, string>; status: string; studentId?: string }
type ReportTab = 'Enrollment' | 'Students' | 'Teachers' | 'Materials' | 'Assessments'

const ReportMetric: FC<{ label: string; value: string | number; detail?: string }> = ({ label, value, detail }) => <Paper elevation={0} sx={{ p: 2.25, borderRadius: 3, height: '100%' }}><Typography variant="body2" color="text.secondary">{label}</Typography><Typography variant="h3" sx={{ mt: 1, fontSize: { xs: 25, md: 30 } }}>{value}</Typography>{detail && <Typography variant="caption" color="text.secondary">{detail}</Typography>}</Paper>

const ReportTable: FC<{ headers: string[]; rows: string[][] }> = ({ headers, rows }) => <TableContainer><Table size="small"><TableHead><TableRow>{headers.map((header) => <TableCell key={header} sx={{ fontWeight: 700 }}>{header}</TableCell>)}</TableRow></TableHead><TableBody>{rows.length ? rows.map((row, index) => <TableRow key={`${row[0]}-${index}`}>{row.map((value, valueIndex) => <TableCell key={`${value}-${valueIndex}`}>{value || '—'}</TableCell>)}</TableRow>) : <TableRow><TableCell colSpan={headers.length}><Typography color="text.secondary">No matching records.</Typography></TableCell></TableRow>}</TableBody></Table></TableContainer>

const ReportsAnalyticsPage: FC = () => {
  ...
  const [teachers, setTeachers] = useState<AdminRecord[]>([])
  const [materials, setMaterials] = useState<AdminRecord[]>([])
  const [assessments, setAssessments] = useState<AdminRecord[]>([])
  ...
  useEffect(() => {
    Promise.all([
      api.get<{ students: ReportStudent[] }>('/api/admin/students', { withCredentials: true }),
      api.get<{ records: AdminRecord[] }>('/api/admin/teachers', { withCredentials: true }),
      api.get<{ records: AdminRecord[] }>('/api/admin/learning-materials', { withCredentials: true }),
      api.get<{ records: AdminRecord[] }>('/api/admin/all-assessments', { withCredentials: true }),
      api.get<{ records: ReportUser[] }>('/api/admin/user-accounts', { withCredentials: true }),
  ...
  const sections = [...new Set(students.filter((student) => grade === 'All' || student.gradeLevel === grade).map((student) => student.classSection).filter(Boolean))]
  const filteredStudents = students.filter((student) => (academicYear === 'All' || student.academicYear === academicYear) && (grade === 'All' || student.gradeLevel === grade) && (classSection === 'All' || student.classSection === classSection))
  const filteredMaterials = materials.filter((material) => (classSection === 'All' || material.data.Class === classSection) && (grade === 'All' || material.data.Class?.includes(grade)))
  const studentUsers = new Set(users.filter((user) => user.data.Role === 'STUDENT').map((user) => user.studentId).filter((studentId): studentId is string => Boolean(studentId)))
  const teacherUsers = new Set(users.filter((user) => user.data.Role === 'TEACHER').map((user) => user.title))
  const grouped = (values: string[]) => Object.entries(values.reduce<Record<string, number>>((counts, value) => { counts[value || '—'] = (counts[value || '—'] || 0) + 1; return counts }, {})).sort(([, left], [, right]) => right - left)
  const exportReport = (format: 'csv' | 'pdf') => {
    if (format === 'pdf') {
      window.print()
      return
    }
    const rows = [['Report', tab], ['Academic Year', academicYear], ['Grade', grade], ['Section', classSection], ['Date Range', dateRange], ['Metric', 'Value'], [tab === 'Enrollment' ? 'Total Enrollments' : tab === 'Students' ? 'Total Students' : tab === 'Teachers' ? 'Total Teachers' : tab === 'Materials' ? 'Total Materials' : 'Total Assessments', String(tab === 'Enrollment' || tab === 'Students' ? filteredStudents.length : tab === 'Teachers' ? teachers.length : tab === 'Materials' ? filteredMaterials.length : assessments.length)]]
    const content = rows.map((row) => row.map((value) => `"${value.replace(/"/g, '""')}"`).join(',')).join('\n')
    const link = document.createElement('a')
    link.href = URL.createObjectURL(new Blob([content], { type: 'text/csv;charset=utf-8' }))
    link.download = `${tab.toLowerCase()}-report.csv`
    link.click()
    URL.revokeObjectURL(link.href)
  }

  const enrollmentContent = <Stack spacing={2}><Grid container spacing={2}><Grid item xs={12} sm={6} md={3}><ReportMetric label="Total enrollments" value={filteredStudents.length} detail="Filtered students" /></Grid><Grid item xs={12} sm={6} md={3}><ReportMetric label="Academic years" value={new Set(filteredStudents.map((student) => student.academicYear)).size} /></Grid><Grid item xs={12} sm={6} md={3}><ReportMetric label="Grade levels" value={new Set(filteredStudents.map((student) => student.gradeLevel)).size} /></Grid><Grid item xs={12} sm={6} md={3}><ReportMetric label="Class / sections" value={new Set(filteredStudents.map((student) => student.classSection)).size} /></Grid></Grid><Paper elevation={0} sx={{ p: 2.5, borderRadius: 3 }}><Typography variant="h6" sx={{ mb: 2 }}>Total Enrollments by Academic Year</Typography><ReportTable headers={['Academic year', 'Enrollments']} rows={grouped(filteredStudents.map((student) => student.academicYear)).map(([label, count]) => [label, String(count)])} /></Paper><Grid container spacing={2}><Grid item xs={12} md={6}><Paper elevation={0} sx={{ p: 2.5, borderRadius: 3 }}><Typography variant="h6" sx={{ mb: 2 }}>Enrollment by Grade Level</Typography><ReportTable headers={['Grade', 'Students']} rows={grouped(filteredStudents.map((student) => student.gradeLevel)).map(([label, count]) => [label, String(count)])} /></Paper></Grid><Grid item xs={12} md={6}><Paper elevation={0} sx={{ p: 2.5, borderRadius: 3 }}><Typography variant="h6" sx={{ mb: 2 }}>Enrollment by Class / Section</Typography><ReportTable headers={['Class / section', 'Students']} rows={grouped(filteredStudents.map((student) => student.classSection)).map(([label, count]) => [label, String(count)])} /></Paper></Grid></Grid><Paper elevation={0} sx={{ p: 2.5, borderRadius: 3 }}><Typography variant="h6">Enrollment Trend Over Time</Typography><Typography color="text.secondary" sx={{ mt: 1 }}>Enrollment trend is represented by the academic-year totals above and updates with the shared filters.</Typography></Paper></Stack>
  const studentsContent = <Stack spacing={2}><Grid container spacing={2}><Grid item xs={12} sm={6} md={3}><ReportMetric label="Total students" value={filteredStudents.length} /></Grid><Grid item xs={12} sm={6} md={3}><ReportMetric label="No guardian linked" value={filteredStudents.filter((student) => !student.guardianLinks?.length).length} /></Grid><Grid item xs={12} sm={6} md={3}><ReportMetric label="No login account" value={filteredStudents.filter((student) => !studentUsers.has(student.id)).length} /></Grid><Grid item xs={12} sm={6} md={3}><ReportMetric label="Active students" value={filteredStudents.filter((student) => student.status === 'Active').length} /></Grid></Grid><Paper elevation={0} sx={{ p: 2.5, borderRadius: 3 }}><Typography variant="h6" sx={{ mb: 2 }}>Total Students by Grade and Section</Typography><ReportTable headers={['Grade', 'Class / section', 'Students']} rows={Object.entries(filteredStudents.reduce<Record<string, number>>((counts, student) => { const key = `${student.gradeLevel} · ${student.classSection}`; counts[key] = (counts[key] || 0) + 1; return counts }, {})).map(([label, count]) => [...label.split(' · '), String(count)])} /></Paper><Paper elevation={0} sx={{ p: 2.5, borderRadius: 3 }}><Typography variant="h6">Student Performance Summary</Typography><Typography color="text.secondary" sx={{ mt: 1 }}>Performance results are not yet linked to student records. This summary will populate when result data is available.</Typography></Paper></Stack>
  const teachersContent = <Stack spacing={2}><Grid container spacing={2}><Grid item xs={12} sm={6} md={3}><ReportMetric label="Total teachers" value={teachers.length} /></Grid><Grid item xs={12} sm={6} md={3}><ReportMetric label="No class assigned" value={teachers.filter((teacher) => !teacher.data['Assigned Classes'] || teacher.data['Assigned Classes'] === '—').length} /></Grid><Grid item xs={12} sm={6} md={3}><ReportMetric label="No login account" value={teachers.filter((teacher) => !teacherUsers.has(teacher.title)).length} /></Grid><Grid item xs={12} sm={6} md={3}><ReportMetric label="Materials + assessments" value={materials.length + assessments.length} /></Grid></Grid><Grid container spacing={2}><Grid item xs={12} md={6}><Paper elevation={0} sx={{ p: 2.5, borderRadius: 3 }}><Typography variant="h6" sx={{ mb: 2 }}>Total Teachers by Subject</Typography><ReportTable headers={['Subject', 'Teachers']} rows={grouped(teachers.flatMap((teacher) => (teacher.data['Assigned Subjects'] || '').split(',').map((subject) => subject.trim()).filter(Boolean))).map(([label, count]) => [label, String(count)])} /></Paper></Grid><Grid item xs={12} md={6}><Paper elevation={0} sx={{ p: 2.5, borderRadius: 3 }}><Typography variant="h6" sx={{ mb: 2 }}>Teacher Activity Summary</Typography><ReportTable headers={['Activity', 'Total']} rows={[[`Materials uploaded`, String(materials.length)], [`Assessments created`, String(assessments.length)]]} /></Paper></Grid></Grid><Paper elevation={0} sx={{ p: 2.5, borderRadius: 3 }}><Typography variant="h6">Teachers by Subject / Grade Assigned</Typography><Typography color="text.secondary" sx={{ mt: 1 }}>Use the subject assignments above together with the shared grade and section filters.</Typography></Paper></Stack>
  const materialsContent = <Stack spacing={2}><Grid container spacing={2}><Grid item xs={12} sm={6} md={3}><ReportMetric label="Total materials uploaded" value={filteredMaterials.length} /></Grid><Grid item xs={12} sm={6} md={3}><ReportMetric label="Teachers uploading" value={new Set(filteredMaterials.map((material) => material.data['Uploaded by'])).size} /></Grid><Grid item xs={12} sm={6} md={3}><ReportMetric label="Subjects covered" value={new Set(filteredMaterials.map((material) => material.data.Subject)).size} /></Grid><Grid item xs={12} sm={6} md={3}><ReportMetric label="Downloadable" value={filteredMaterials.filter((material) => material.data.Downloadable === 'Yes').length} /></Grid></Grid><Grid container spacing={2}><Grid item xs={12} md={6}><Paper elevation={0} sx={{ p: 2.5, borderRadius: 3 }}><Typography variant="h6" sx={{ mb: 2 }}>Materials by Teacher</Typography><ReportTable headers={['Teacher', 'Materials']} rows={grouped(filteredMaterials.map((material) => material.data['Uploaded by'] || '—')).map(([label, count]) => [label, String(count)])} /></Paper></Grid><Grid item xs={12} md={6}><Paper elevation={0} sx={{ p: 2.5, borderRadius: 3 }}><Typography variant="h6" sx={{ mb: 2 }}>Materials by Subject / Class</Typography><ReportTable headers={['Subject', 'Class', 'Materials']} rows={Object.entries(filteredMaterials.reduce<Record<string, number>>((counts, material) => { const key = `${material.data.Subject} · ${material.data.Class}`; counts[key] = (counts[key] || 0) + 1; return counts }, {})).map(([label, count]) => [...label.split(' · '), String(count)])} /></Paper></Grid></Grid><Paper elevation={0} sx={{ p: 2.5, borderRadius: 3 }}><Typography variant="h6">Upload Trend Over Time</Typography><Typography color="text.secondary" sx={{ mt: 1 }}>Upload dates are available in the Learning Materials records and respond to the shared filters.</Typography></Paper></Stack>
  const assessmentsContent = <Stack spacing={2}><Grid container spacing={2}><Grid item xs={12} sm={6} md={3}><ReportMetric label="Total assessments created" value={assessments.length} /></Grid><Grid item xs={12} sm={6} md={3}><ReportMetric label="Teachers creating" value={new Set(assessments.map((assessment) => assessment.data.Teacher)).size} /></Grid><Grid item xs={12} sm={6} md={3}><ReportMetric label="Assessment types" value={new Set(assessments.map((assessment) => assessment.data['Assessment Type'])).size} /></Grid><Grid item xs={12} sm={6} md={3}><ReportMetric label="Pending approvals" value={assessments.filter((assessment) => assessment.status === 'Draft').length} /></Grid></Grid><Grid container spacing={2}><Grid item xs={12} md={6}><Paper elevation={0} sx={{ p: 2.5, borderRadius: 3 }}><Typography variant="h6" sx={{ mb: 2 }}>Assessments by Teacher</Typography><ReportTable headers={['Teacher', 'Assessments']} rows={grouped(assessments.map((assessment) => assessment.data.Teacher || '—')).map(([label, count]) => [label, String(count)])} /></Paper></Grid><Grid item xs={12} md={6}><Paper elevation={0} sx={{ p: 2.5, borderRadius: 3 }}><Typography variant="h6" sx={{ mb: 2 }}>Assessments by Type</Typography><ReportTable headers={['Type', 'Assessments']} rows={grouped(assessments.map((assessment) => assessment.data['Assessment Type'] || '—')).map(([label, count]) => [label, String(count)])} /></Paper></Grid></Grid><Paper elevation={0} sx={{ p: 2.5, borderRadius: 3 }}><Typography variant="h6">Average Score by Class / Subject</Typography><Typography color="text.secondary" sx={{ mt: 1 }}>Score data is not yet linked to assessment records. Pending Result Approvals currently counts draft assessments.</Typography></Paper></Stack>
  const content = tab === 'Enrollment' ? enrollmentContent : tab === 'Students' ? studentsContent : tab === 'Teachers' ? teachersContent : tab === 'Materials' ? materialsContent : assessmentsContent

  return <AdminPanelLayout title="Reports & Analytics"><Container maxWidth="xl" sx={{ py: { xs: 4, md: 6 } }}><Breadcrumbs aria-label="breadcrumb" sx={{ mb: 2 }}><Typography variant="subtitle2" color="text.secondary">Admin</Typography><Typography variant="subtitle2" color="primary.main">Reports & Analytics</Typography></Breadcrumbs><Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', sm: 'center' }} spacing={2} sx={{ mb: 3 }}><Box><Typography component="h1" variant="h1" sx={{ fontSize: { xs: 30, md: 38 }, mb: 0.5 }}>Reports & Analytics</Typography><Typography color="text.secondary">Review enrollment, student, teacher, material, and assessment insights in one filtered view.</Typography></Box><Stack direction="row" spacing={1}><Button variant="outlined" startIcon={<DownloadRounded />} onClick={() => exportReport('csv')}>Export CSV</Button><Button variant="contained" startIcon={<DownloadRounded />} onClick={() => exportReport('pdf')}>Export PDF</Button></Stack></Stack><Paper elevation={0} sx={{ p: { xs: 2, md: 2.5 }, borderRadius: 3, mb: 2 }}><Grid container spacing={1.5}><Grid item xs={12} sm={6} md={3}><TextField fullWidth size="small" select label="Academic Year" value={academicYear} onChange={(event) => setAcademicYear(event.target.value)}><MenuItem value="All">All academic years</MenuItem>{academicYears.map((year) => <MenuItem key={year} value={year}>{year}</MenuItem>)}</TextField></Grid><Grid item xs={12} sm={6} md={3}><TextField fullWidth size="small" select label="Grade" value={grade} onChange={(event) => { setGrade(event.target.value); setClassSection('All') }}><MenuItem value="All">All grades</MenuItem>{grades.map((value) => <MenuItem key={value} value={value}>{value}</MenuItem>)}</TextField></Grid><Grid item xs={12} sm={6} md={3}><TextField fullWidth size="small" select label="Section" value={classSection} onChange={(event) => setClassSection(event.target.value)}><MenuItem value="All">All sections</MenuItem>{sections.map((value) => <MenuItem key={value} value={value}>{value}</MenuItem>)}</TextField></Grid><Grid item xs={12} sm={6} md={3}><TextField fullWidth size="small" select label="Date Range" value={dateRange} onChange={(event) => setDateRange(event.target.value)}><MenuItem value="All time">All time</MenuItem><MenuItem value="Last 30 days">Last 30 days</MenuItem><MenuItem value="Last 90 days">Last 90 days</MenuItem><MenuItem value="This academic year">This academic year</MenuItem></TextField></Grid></Grid></Paper><Paper elevation={0} sx={{ borderRadius: 3, mb: 2, overflow: 'hidden' }}><Stack direction={{ xs: 'column', md: 'row' }} spacing={0.5} sx={{ p: 1, borderBottom: 1, borderColor: 'divider' }}>{(['Enrollment', 'Students', 'Teachers', 'Materials', 'Assessments'] as ReportTab[]).map((item) => <Button key={item} onClick={() => setTab(item)} variant={tab === item ? 'contained' : 'text'}>{item}</Button>)}</Stack></Paper>{loading ? <LinearProgress /> : error ? <Alert severity="error">{error}</Alert> : content}</Container></AdminPanelLayout>
}
...
const AdminManagementPage: FC = () => {
  ...
  useEffect(() => {
    if (section !== 'user-accounts') return
    api.get<{ students: { id: string; fullName: string; admissionNumber: string }[] }>('/api/admin/students', { withCredentials: true })
      .then(({ data }) => setCreateOptions(data.students.map(({ id, fullName, admissionNumber }) => ({ id, name: `${fullName} · ${admissionNumber}` }))))
      .catch(() => setError('Unable to load student records. Please try again.'))
  }, [section])
  ...
  useEffect(() => {
    ...
    api.get<{ records: AdminRecord[] }>(`/api/admin/${section}`, { withCredentials: true })
      .then(({ data }) => setRecords(section === 'learning-materials' ? data.records.map((record) => {
        const materialData = record.data as Record<string, unknown>
        const studentCount = Array.isArray(materialData.studentIds) ? materialData.studentIds.length : 0
        return { ...record, data: { Material: record.title, File: String(materialData.fileName || '—'), 'File type': String(materialData.fileExtension || '—').toUpperCase(), Subject: String(materialData.subjectName || '—'), Class: String(materialData.className || '—'), 'Uploaded by': String(materialData.teacherName || '—'), Audience: materialData.assignmentScope === 'Whole Class' ? 'Whole class' : `${studentCount} selected student${studentCount === 1 ? '' : 's'}`, Description: String(materialData.description || '—'), 'Uploaded date': record.createdAt ? new Intl.DateTimeFormat(undefined, { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(record.createdAt)) : '—', Downloadable: 'Yes', materialId: record.id } }
      }) : data.records))
    ...
    : section === 'teachers'
      ...
      : section === 'grade-levels'
        ...
        : section === 'classes-sections'
          ...
          : section === 'subjects'
            ? [{ name: 'title', label: 'Subject' }, { name: 'gradeLevels', label: 'Grade levels' }, { name: 'teachers', label: 'Teachers' }, { name: 'status', label: 'Status', options: ['Draft', 'Published'] }]
      ...
      : section === 'teachers'
        ...
        : section === 'grade-levels'
          ...
          : section === 'classes-sections'
            ...
            : section === 'subjects'
              ? { title: record.title, gradeLevels: record.data['Grade levels'] || '', teachers: record.data.Teachers || '', status: record.status }
  ...
  const handleEditSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const payload = section === 'subjects'
        ? { title: editValues.title, data: { 'Grade levels': editValues.gradeLevels || '—', Teachers: editValues.teachers || '—' }, status: editValues.status }
        : editValues
  ...
  const handleCreateFormSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!title) return
    if (section === 'user-accounts' && createValues.role === 'STUDENT' && !createValues.studentId) {
      setError('Select the student record this account belongs to.')
      return
    }
  ...
  if (section === 'reports-analytics') return <ReportsAnalyticsPage />
          ...
          {isCreateFormOpen && config.createFields && <Box component="form" onSubmit={handleCreateFormSubmit} sx={{ p: 1, mb: 1 }}>
            <Grid container spacing={1.5}>
              {config.createFields.map((field) => {
                if (field.name === 'studentId' && createValues.role !== 'STUDENT') return null
                const options = field.name === 'gradeLevelId' || field.name === 'studentId' ? createOptions : field.options
                return <Grid item xs={12} sm={field.multiple ? 12 : 6} key={field.name}>
                  {field.multiple ? <Box>
                  ...
                  </Box> : <TextField fullWidth size="small" label={field.label} type={options ? undefined : field.type || 'text'} select={Boolean(options)} required={field.required} value={field.type === 'file' ? undefined : createValues[field.name] || ''} onChange={(event) => setCreateValues((current) => ({ ...current, [field.name]: field.type === 'file' ? (event.target as HTMLInputElement).files?.[0]?.name || '' : event.target.value, ...(field.name === 'role' && event.target.value !== 'STUDENT' ? { studentId: '' } : {}) }))}>{options?.map((option) => <MenuItem key={typeof option === 'string' ? option : option.id} value={typeof option === 'string' ? option : option.id}>{typeof option === 'string' ? option : option.name}</MenuItem>)}</TextField>}
                </Grid>
              })}
              <Grid item xs={12}><Button type="submit" variant="contained">Save</Button></Grid>
            </Grid>
          </Box>}
          {editingRecordId && editableSections && <Box component="form" onSubmit={handleEditSubmit} sx={{ p: 1, mb: 1 }}><Typography variant="subtitle1" sx={{ mb: 1 }}>Edit {config.title.slice(0, -5)}</Typography><Grid container spacing={1.5}>{editFields.map((field) => { const usesGradeLevelOptions = section === 'classes-sections' && field.name === 'gradeLevelId'; const options = usesGradeLevelOptions ? createOptions : field.options; return <Grid item xs={12} sm={field.name === 'address' || field.name === 'assignedSubjects' || field.name === 'assignedClasses' ? 12 : 6} key={field.name}><TextField fullWidth size="small" label={field.label} type={field.type || 'text'} select={Boolean(options)} value={editValues[field.name] || ''} onChange={(event) => setEditValues((current) => ({ ...current, [field.name]: event.target.value }))}>{options?.map((option) => <MenuItem key={typeof option === 'string' ? option : option.id} value={typeof option === 'string' ? option : option.id}>{typeof option === 'string' ? option : option.name}</MenuItem>)}</TextField></Grid> })}<Grid item xs={12}><Stack direction="row" spacing={1}><Button type="submit" variant="contained" disabled={editSaving}>{editSaving ? 'Saving...' : 'Save changes'}</Button><Button type="button" onClick={() => { setEditingRecordId(''); setEditValues({}) }}>Cancel</Button></Stack></Grid></Grid></Box>}
