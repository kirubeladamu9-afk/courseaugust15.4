...
const ReportsAnalyticsPage: FC = () => {
  ...
  const exportReport = () => {
    const content = `${tab} Report\nAcademic Year: ${academicYear}\nGrade: ${grade}\nSection: ${classSection}\nDate Range: ${dateRange}\n\n${tab === 'Enrollment' ? `Total Enrollments: ${filteredStudents.length}` : tab === 'Students' ? `Total Students: ${filteredStudents.length}` : tab === 'Teachers' ? `Total Teachers: ${teachers.length}` : tab === 'Materials' ? `Total Materials: ${filteredMaterials.length}` : `Total Assessments: ${assessments.length}`}`
    const link = document.createElement('a')
    link.href = URL.createObjectURL(new Blob([content], { type: 'text/plain;charset=utf-8' }))
    link.download = `${tab.toLowerCase()}-report.txt`
    link.click()
