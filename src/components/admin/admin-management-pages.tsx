...
type ReportStudent = { fullName: string; gradeLevel: string; classSection: string; academicYear: string; status: string; createdAt?: string; guardianLinks: { guardian: { name: string } }[] }
type ReportUser = { title: string; data: Record<string, string>; status: string }
...
const ReportsAnalyticsPage: FC = () => {
  ...
  const studentUsers = new Set(users.filter((user) => user.data.Role === 'STUDENT').map((user) => user.title))
  ...
  const studentsContent = <Stack spacing={2}><Grid container spacing={2}><Grid item xs={12} sm={6} md={3}><ReportMetric label="Total students" value={filteredStudents.length} /></Grid><Grid item xs={12} sm={6} md={3}><ReportMetric label="No guardian linked" value={filteredStudents.filter((student) => !student.guardianLinks?.length).length} /></Grid><Grid item xs={12} sm={6} md={3}><ReportMetric label="No login account" value={filteredStudents.filter((student) => !studentUsers.has(student.fullName)).length} /></Grid><Grid item xs={12} sm={6} md={3}><ReportMetric label="Active students" value={filteredStudents.filter((student) => student.status === 'Active').length} /></Grid></Grid><Paper elevation={0} sx={{ p: 2.5, borderRadius: 3 }}><Typography variant="h6" sx={{ mb: 2 }}>Total Students by Grade and Section</Typography><ReportTable headers={['Grade', 'Class / section', 'Students']} rows={Object.entries(filteredStudents.reduce<Record<string, number>>((counts, student) => { const key = `${student.gradeLevel} · ${student.classSection}`; counts[key] = (counts[key] || 0) + 1; return counts }, {})).map(([label, count]) => [...label.split(' · '), String(count)])} /></Paper><Paper elevation={0} sx={{ p: 2.5, borderRadius: 3 }}><Typography variant="h6">Student Performance Summary</Typography><Typography color="text.secondary" sx={{ mt: 1 }}>Performance results are not yet linked to student records. This summary will populate when result data is available.</Typography></Paper></Stack>
