...
const InteractiveDiagramViewer: FC<InteractiveDiagramViewerProps> = ({ imageUrl, hotspots, onViewed, onHotspotOpen }) => {
  ...
  useEffect(() => {
    ...
    if (!image) return
    const updateImageSize = () => setImageSize({ width: image.clientWidth, height: image.clientHeight })
    updateImageSize()
  ...
  const pixelPosition = (position: string, dimension: number) => `${(Number.parseFloat(position) / 100) * dimension}px`
