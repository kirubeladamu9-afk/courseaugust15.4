...
const DashboardSidebar: FC<SidebarProps> = ({ activeView, onSelectView }) => {
  ...
  const user = getAuthenticatedUser()
  const displayName = user?.name || 'Alex Morgan'
  const navItem = (view: DashboardView, label: string, icon: ReactNode, active: boolean, endIcon?: ReactNode, onClick?: () => void, ariaExpanded?: boolean) => <Box component="button" type="button" aria-expanded={ariaExpanded} onClick={onClick ?? (() => onSelectView(view))} sx={{ width: '100%', display: 'flex', alignItems: 'center', gap: 1.25, border: 0, borderRadius: 2.5, px: 1.5, py: 1.05, mb: .45, backgroundColor: active ? '#127C71' : 'transparent', color: active ? '#fff' : '#65717f', cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left', transition: 'all .18s ease', boxShadow: active ? '0 7px 16px rgba(93,113,220,.18)' : 'none', '&:hover': { backgroundColor: active ? '#0D6A69' : '#eef1fa', color: active ? '#fff' : '#0D6A69', transform: active ? 'none' : 'translateX(2px)' } }}><Box sx={{ display: 'flex', opacity: active ? 1 : .82 }}>{icon}</Box><Typography variant="body2" sx={{ flex: 1, fontWeight: active ? 700 : 500 }}>{label}</Typography>{endIcon}</Box>
  ...
  return <Box sx={{ width: drawerWidth, height: '100%', overflowY: 'auto', backgroundColor: '#fff', display: 'flex', flexDirection: 'column', borderRight: 1, borderColor: 'rgba(41, 75, 91, .08)' }}>
    <Box sx={{ px: 3, pt: 2.5, pb: 2.25, borderBottom: '1px solid rgba(18,124,113,.12)' }}><Box sx={{ transform: 'scale(.82)', transformOrigin: 'left center', width: '122%' }}><Logo /></Box><Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: .2 }}>Your personal learning space</Typography></Box>
    ...
    <Box sx={{ p: 1.75, pt: 1, borderTop: '1px solid rgba(18,124,113,.12)' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.2, p: 1.25, mb: 1.25, borderRadius: 3, background: 'linear-gradient(135deg, rgba(18,124,113,.10), rgba(18,124,113,.04))' }}><Box sx={{ position: 'relative' }}><Avatar sx={{ width: 34, height: 34, bgcolor: '#127C71', fontSize: 14 }}>{displayName.charAt(0)}</Avatar><Box sx={{ position: 'absolute', right: -1, bottom: 0, width: 9, height: 9, borderRadius: '50%', bgcolor: '#55c58b', border: '2px solid rgba(18,124,113,.10)' }} /></Box><Box sx={{ minWidth: 0 }}><Typography variant="body2" noWrap sx={{ fontWeight: 800 }}>{displayName}</Typography><Typography variant="caption" color="text.secondary">Student account</Typography></Box></Box>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25, px: 1, py: .7 }}><SchoolOutlinedIcon sx={{ color: '#127C71' }} fontSize="small" /><Typography variant="caption" color="text.secondary">Keep learning at your own pace.</Typography></Box>
    </Box>
  </Box>
}
...
const OverviewView: FC<{ enrollments: DashboardEnrollment[]; now: Date; onSelectView: (view: DashboardView) => void; onOpenCourse: (courseId: number, lessonId?: number) => void; gamificationData: GamificationData }> = ({ enrollments, now, onSelectView, onOpenCourse, gamificationData }) => {
  ...
  return <>
    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: { xs: 'flex-start', sm: 'center' }, gap: 2, mb: 3 }}>
      <Box><Typography variant="body2" sx={{ color: '#127C71', fontWeight: 800, mb: .5 }}>WELCOME BACK, {displayName.toUpperCase()}</Typography><Typography component="h1" variant="h4" sx={{ fontWeight: 800, letterSpacing: '-.04em' }}>Let&apos;s make learning fun!</Typography><Typography color="text.secondary" variant="body2" sx={{ mt: .75 }}>Here&apos;s a clear view of your learning journey.</Typography></Box>
    ...
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: 'minmax(250px, .85fr) minmax(360px, 1.45fr) minmax(250px, .85fr)' }, gap: 2.25, alignItems: 'start' }}>
      <Stack spacing={2.25}>
        <Paper elevation={0} sx={{ p: 2.25, borderRadius: 4, background: 'linear-gradient(135deg,#f8f9ff,#eff1ff)', border: '1px solid #e3e7ff' }}><Stack direction="row" justifyContent="space-between" alignItems="center"><Box><Typography variant="body2" color="text.secondary">Ongoing classes</Typography><Typography variant="h3" sx={{ mt: .3, fontWeight: 800 }}>{activeCourseCount}</Typography></Box><Box sx={{ p: 1, color: '#127C71', bgcolor: '#fff', borderRadius: 2 }}><MenuBookOutlinedIcon /></Box></Stack><Typography variant="caption" color="text.secondary">Courses currently in progress</Typography></Paper>
        {courseEnrollments.slice(0, 2).map((enrollment, index) => enrollment.course && <Paper key={enrollment.id} elevation={0} sx={{ p: 2, borderRadius: 4, bgcolor: index ? 'rgba(159,241,210,.24)' : 'rgba(18,124,113,.08)', border: '1px solid rgba(93,113,220,.12)' }}><Stack direction="row" justifyContent="space-between"><Box sx={{ p: .75, borderRadius: 2, bgcolor: '#fff', color: '#127C71' }}><MenuBookOutlinedIcon fontSize="small" /></Box><MoreHorizIcon color="action" fontSize="small" /></Stack><Typography sx={{ fontWeight: 800, mt: 1.5 }}>{enrollment.course.title}</Typography><Stack direction="row" spacing={.5} sx={{ mt: .75 }}><Chip label={enrollment.course.category} size="small" /><Chip label={enrollment.course.level} size="small" variant="outlined" /></Stack><Box sx={{ mt: 2 }}><Stack direction="row" justifyContent="space-between"><Typography variant="caption" color="text.secondary">In progress</Typography><Typography variant="caption" sx={{ fontWeight: 800 }}>{enrollment.progress}%</Typography></Stack><LinearProgress variant="determinate" value={enrollment.progress} sx={{ mt: .6, height: 6, borderRadius: 5, bgcolor: '#fff' }} /></Box></Paper>)}
      </Stack>
      <Stack spacing={2.25}>
        <Paper elevation={0} sx={{ p: 2.5, borderRadius: 4, border: '1px solid #edf0f3' }}><Stack direction="row" justifyContent="space-between"><Box><Typography component="h2" variant="h6" sx={{ fontWeight: 800 }}>Performance chart</Typography><Stack direction="row" spacing={1.25} sx={{ mt: .8 }}><Typography variant="caption" color="text.secondary">● Theory</Typography><Typography variant="caption" sx={{ color: '#9FF1D2' }}>● Practice</Typography><Typography variant="caption" sx={{ color: '#e7b84e' }}>● Lessons</Typography></Stack></Box><Chip label="Weekly" size="small" variant="outlined" /></Stack><Box sx={{ height: 220, mt: 2, position: 'relative', overflow: 'hidden', borderBottom: '1px solid #e7ebef', backgroundImage: 'linear-gradient(#f2f4f6 1px, transparent 1px)', backgroundSize: '100% 54px' }}><Box sx={{ position: 'absolute', bottom: 26, left: 0, right: 0, height: 140, borderTop: '3px solid #127C71', borderRadius: '50% 50% 0 0 / 75% 75% 0 0', transform: 'skewY(-10deg)' }} /><Box sx={{ position: 'absolute', bottom: 22, left: 0, right: 0, height: 105, borderTop: '3px solid #ffaf35', borderRadius: '50% 50% 0 0 / 75% 75% 0 0', transform: 'skewY(-5deg)' }} /><Stack direction="row" justifyContent="space-between" sx={{ position: 'absolute', bottom: 0, left: 0, right: 0 }}>{['Sat', 'Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri'].map((day) => <Typography key={day} variant="caption" color="text.secondary">{day}</Typography>)}</Stack></Box></Paper>
