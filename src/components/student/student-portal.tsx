...
const pageDetails: Record<string, { title: string; description: string }> = {
  ...
  settings: { title: 'Settings', description: 'Update your contact details and password.' },
}
...
const Dashboard: FC<{ data: StudentDashboardData | null; error: string }> = ({ data, error }) => {
  ...
  const answerText = (answer: string | string[]) => Array.isArray(answer) ? answer.join(', ') || 'Not answered' : answer || 'Not answered'
...
const Settings: FC = () => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setSaving(true)
    try {
      const { data } = await api.post<{ message: string }>('/api/student/change-password', { currentPassword, newPassword }, { withCredentials: true })
