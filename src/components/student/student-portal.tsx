...
const Dashboard: FC<{ data: StudentDashboardData | null; error: string }> = ({ data, error }) => { if (error) return <Alert severity="error">{error}</Alert>; if (!data) return <LinearProgress />; const { student, todayTimetable, upcomingAssessments, recentResults, announcements: latestAnnouncements } = data; return <Stack spacing={2}>
...
type StudentResultQuestion = { prompt: string; correctAnswer: string | string[] }
