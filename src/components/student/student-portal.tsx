...
const Assessments: FC = () => {
  ...
  return <Stack spacing={2}>{loading && <LinearProgress />}{error && <Alert severity="error">{error}</Alert>}<Paper elevation={0} sx={{ p: { xs: 2, md: 3 }, borderRadius: 3 }}><Typography variant="h5">Upcoming Assessments</Typography>{!loading && !error && (assessments.length ? <Stack spacing={1.5} sx={{ mt: 2 }}>{assessments.map((item) => { const endsAt = new Date(item.endsAt).getTime(); const dueAt = new Date(item.dueDate).getTime(); const startsAt = new Date(item.startsAt).getTime(); const available = now >= startsAt && now <= endsAt; const notYetAvailable = now < startsAt; const remainingSeconds = Math.max(0, Math.floor((endsAt - now) / 1000)); return <Box key={item.id} sx={{ p: 2, borderRadius: 2, backgroundColor: 'background.default' }}><Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', sm: 'center' }} spacing={1}><Box><Typography fontWeight={700}>{item.title}</Typography><Typography variant="body2" color="text.secondary">{item.subjectName || item.assessmentType} · {item.assessmentType} · {item.timeLimitMinutes} min · Starts {formatDateTime(item.startsAt)} · {item.questionCount} question{item.questionCount === 1 ? '' : 's'}</Typography></Box>{item.status === 'Submitted' ? <Chip label="Submitted" color="success" size="small" /> : !item.questionCount || !available ? <Chip label={notYetAvailable ? `Available ${formatDateTime(item.startsAt)}` : 'Closed'} color={notYetAvailable ? 'warning' : 'default'} size="small" /> : <Stack direction="row" spacing={1} alignItems="center"><Chip label={`${formatRemainingTime(remainingSeconds)} remaining`} color="primary" size="small" /><Button variant="contained" startIcon={<PlayCircleOutlineRounded />} onClick={() => setSelectedAssignmentId(item.id)}>Start</Button></Stack>}</Stack></Box> })}</Stack> : <Typography color="text.secondary" sx={{ mt: 2 }}>No active assessments are assigned to your class.</Typography>)}</Paper><Paper elevation={0} sx={{ p: { xs: 2, md: 3 }, borderRadius: 3 }}><Typography variant="h5">My Results</Typography>{!loading && !error && (results.length ? <Stack spacing={1.5} sx={{ mt: 2 }}>{results.map((result) => <Stack key={result.id + '-' + result.submittedAt} direction="row" justifyContent="space-between" alignItems="center" role="button" tabIndex={0} onClick={() => setSelectedResult(result)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') setSelectedResult(result) }} sx={{ cursor: 'pointer', borderRadius: 2, p: 1, '&:hover': { backgroundColor: 'action.hover' } }}><Box><Typography fontWeight={700}>{result.title}</Typography><Typography variant="caption" color="text.secondary">{result.subjectName || result.assessmentType} · {result.submittedAt ? formatDate(result.submittedAt) : 'Date unavailable'}</Typography></Box><Chip label={result.score === null ? 'Awaiting grading' : result.score + ' / ' + result.totalPoints} size="small" color={result.score === null ? 'warning' : 'success'} /></Stack>)}</Stack> : <Typography color="text.secondary" sx={{ mt: 2 }}>No results to show yet.</Typography>)}</Paper><Paper elevation={0} sx={{ p: { xs: 2, md: 3 }, borderRadius: 3 }}><Typography variant="h5">My Results</Typography>{results.length ? <Stack spacing={2} sx={{ mt: 2 }}>{results.map((result) => { const percentage = result.totalPoints ? Math.round(((result.score || 0) / result.totalPoints) * 100) : 0; return <Box key={`${result.id}-${result.submittedAt}`} sx={{ p: 2, borderRadius: 2, backgroundColor: 'background.default' }}><Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" spacing={1}><Box><Typography fontWeight={700}>{result.title}</Typography><Typography variant="body2" color="text.secondary">{result.subjectName || result.assessmentType}{result.submittedAt ? ` · ${formatDate(result.submittedAt)}` : ''}</Typography></Box><Stack direction="row" spacing={1} alignItems="center"><Chip label={result.status} color="success" size="small" /><Chip label={`${result.score ?? 0} / ${result.totalPoints} · ${percentage}%`} color="primary" size="small" /></Stack></Stack><Stack spacing={1} sx={{ mt: 2 }}>{result.questions.map((question, index) => <Box key={`${result.id}-question-${index}`}><Typography variant="body2" fontWeight={700}>Question {index + 1}: {question.prompt}</Typography><Typography variant="body2" color="success.main">Correct answer: {Array.isArray(question.correctAnswer) ? question.correctAnswer.join(', ') : question.correctAnswer}</Typography></Box>)}</Stack></Box> })}</Stack> : <Typography color="text.secondary" sx={{ mt: 2 }}>No submitted assessments yet.</Typography>}<Dialog open={Boolean(selectedResult)} onClose={() => setSelectedResult(null)} fullWidth maxWidth="md"><DialogTitle>{selectedResult?.title} · Result details</DialogTitle><DialogContent dividers><Stack spacing={2}>{selectedResult?.questions.map((question, index) => <Paper key={`${selectedResult.id}-question-${index}`} variant="outlined" sx={{ p: 2, borderRadius: 2 }}><Typography variant="subtitle1" fontWeight={700}>Question {index + 1}</Typography><Typography sx={{ mt: 0.75 }}>{question.prompt}</Typography><Typography variant="body2" sx={{ mt: 1.5 }}><strong>Your answer:</strong> {Array.isArray(question.studentAnswer) ? question.studentAnswer.join(', ') || 'No answer' : question.studentAnswer || 'No answer'}</Typography><Typography variant="body2" sx={{ mt: 0.5 }}><strong>Correct answer:</strong> {Array.isArray(question.correctAnswer) ? question.correctAnswer.join(', ') : question.correctAnswer || 'No answer'}</Typography></Paper>)}</Stack></DialogContent><DialogActions><Button onClick={() => setSelectedResult(null)}>Close</Button></DialogActions></Dialog></Paper></Stack>
}
