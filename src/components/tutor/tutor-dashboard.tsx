...
const TutorDashboard = ({ darkMode, onToggleDarkMode }: { darkMode: boolean; onToggleDarkMode: () => void }) => {
  ...
  return <Box sx={{ minHeight: '100vh', backgroundColor: 'background.default' }}>
    ...
    <Box sx={{ maxWidth: 1440, mx: 'auto', p: { xs: 2, md: 4 } }}>
      ...
      {view === 'overview' && overview && <><Typography variant="h4" sx={{ mb: 2.25 }}>Teaching overview</Typography><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr 1fr', md: 'repeat(4, 1fr)' }, gap: { xs: 1.25, md: 1.5 }, mb: 1.5 }}>{statItems(overview).map((item) => <Card key={item.label} elevation={0} sx={{ border: '1px solid', borderColor: 'rgba(18,124,113,.12)', borderRadius: 3, backgroundColor: 'rgba(255,255,255,.84)' }}><CardContent sx={{ p: { xs: 1.5, md: 2 }, '&:last-child': { pb: { xs: 1.5, md: 2 } } }}><Stack direction="row" justifyContent="space-between" alignItems="center"><Typography color="text.secondary" variant="body2">{item.label}</Typography>{item.icon}</Stack><Typography variant="h4" sx={{ mt: .75, fontWeight: 800 }}>{item.value}</Typography></CardContent></Card>)}</Box><AtRiskStudentsPanel students={atRiskStudents} loading={atRiskLoading} error={atRiskError} /><Paper elevation={0} sx={{ mt: 2, p: 3, borderRadius: 3, border: '1px solid', borderColor: 'rgba(18,124,113,.12)' }}><Typography variant="h6" sx={{ mb: 1 }}>Your teaching assignments</Typography><Typography color="text.secondary">Manage class rosters, attendance, and your profile from the sections above.</Typography></Paper></>}
