...
const StudentProgressView = ({ target, students, studentsLoading, onBack }: { target: ProgressTarget; students: TutorClassStudent[]; studentsLoading: boolean; onBack: () => void }) => {
...
const TutorDashboard = ({ darkMode, onToggleDarkMode }: { darkMode: boolean; onToggleDarkMode: () => void }) => {
  ...
  return <Box sx={{ minHeight: '100vh', backgroundColor: 'background.default' }}>
    ...
    <Box sx={{ maxWidth: 1440, mx: 'auto', p: { xs: 2, md: 4 } }}>
      ...
      {progressTarget && <StudentProgressView target={progressTarget} students={students} studentsLoading={studentsLoading} onBack={() => setProgressTarget(null)} />}
